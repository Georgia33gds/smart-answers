...
class FlowTestHelperTest < ActiveSupport::TestCase
  ...
  test "caches current_state" do
    state_resolver = stub("state_resolver", from_params: :state)
    SmartAnswer::ResolveState.expects(:new).once.returns(state_resolver)
  ...
  test "busts current_state cache when responses change" do
    state_resolver = stub("state_resolver", from_params: :state)
    SmartAnswer::ResolveState.expects(:new).twice.returns(state_resolver)
    responses = %i[yes no]
