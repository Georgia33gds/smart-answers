class FlowController < ApplicationController
  ...
  def show
    state = SmartAnswer::ResolveState.new(flow).from_response_store(response_store, node_name)
    ...
    if params[:node_slug] == @presenter.node_slug
      ...
      redirect_to flow_path(id: params[:id],
                            ...
                            params: @presenter.forwarding_responses)
    end
  end

  def update
    response_store.add(node_name, params[:response])
    state = SmartAnswer::ResolveState.new(flow).from_response_store(response_store)
    presenter = FlowPresenter.new(flow, state)
    redirect_to flow_path(id: params[:id],
                          node_slug: presenter.node_slug,
                          params: presenter.forwarding_responses)
